...
class SortAreaBlocksTest extends PageTestCase
{
     ...
     */
    public function provideInvalidBlockIDs(): array
    {
        return [
            'a missing block' => [[0, 1]],
    ...
    /**
     * @dataProvider provideInvalidBlockIDs
     *
    ...
    private function addBlocks(Page $page, int $numBlocks, string $areaHandle = 'Main'): array
    {
        ...
        for ($index = 1; $index <= $numBlocks; $index++) {
            ...
            $command->setData($handler->getSaveArgumentsFromApiValue(['fID' => 0, 'altText' => "Block #{$index}"], null));
