...
class AddBlockToPageCommandHandler
{
    ...
    public function __invoke(AddBlockToPageCommand $command)
    {
        ...
        if (!$blockType->includeAll()) {
            ...
            $nb = $nvc->addBlock($blockType, $areaToModify, $data, $command->getSaveMode());
        } else {
            // if we apply to all, then we don't worry about a new version of the page
            $nb = $pageToModify->addBlock($blockType, $areaToModify, $data, $command->getSaveMode());
        }
    ...
    /**
     * Get the block that comes just before another one in a page area.
     *
     * @param \Concrete\Core\Area\Area|string $area
     *
     * @throws \Concrete\Core\Block\Exception\BlockNotFoundException if the block is not in that page area
     *
     * @return \Concrete\Core\Block\Block|null NULL when it's the first block of the area
     */
